...
async def full_cv_pipeline(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Step 2 — Parse resumes in parallel (1 LLM call per resume for skill extraction)
...
def _run_pipeline_background(job_id: str, tmp_path: str, profile_str: str, db_job_id, top_n: int):
    db = SessionLocal()
    try:
        _set_job(job_id, message="Building candidate personas…")
        profile_dict = json.loads(profile_str)
        personas = build_personas(profile_dict)

        _set_job(job_id, message="Extracting resumes from file…")
        raw_resumes = _extract_resumes_from_files([tmp_path])
        if not raw_resumes:
            _set_job(job_id, status="failed", error="No valid resumes found in the uploaded file.")
            return

        total = len(raw_resumes)
        _set_job(job_id, message=f"Parsing {total} resumes…")
        parsed_resumes = _parse_resumes_parallel(raw_resumes)
        raw_text_map = {r["candidate_id"]: r["raw_text"] for r in parsed_resumes}

        _set_job(job_id, message=f"Pre-filtering {total} resumes with embeddings…")
        qualified, rejected = _prefilter(parsed_resumes, profile_dict, SIMILARITY_THRESHOLD)

        _set_job(job_id, message=f"Evaluating {len(qualified)} qualified candidates with AI… (this takes a while)")
        evaluations = _evaluate_parallel(qualified, personas)
        ...
        _set_job(job_id, message="Ranking candidates…")
        ranking = rank_candidates(evaluations, top_n=top_n)

        db_summary = None
        if db_job_id is not None:
            _set_job(job_id, message="Saving results to database…")
        ...
        print(f"[CV_ASYNC] Job {job_id} failed: {e}")
...
async def full_cv_pipeline_async(
    ...
    tmp_path = f"/tmp/cv_upload_{eval_job_id}{suffix}"
